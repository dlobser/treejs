...
TREE.prototype.makeInfo = function(args){
	...
	for (var i = 0; i < args.length; i+=2) {
		...
		// 	info.push(this.makeListAll(args[q]));
		// else
...
TREE.prototype.processInfo = function(array){
...
TREE.prototype.makeList = function(range,stacker,stackArray,index) {
	...
	if(index < range.length){
		...
		else if(range[i] == -2 && index==range.length-1 || range[i] == "all" && index==range.length-1 || range[i] == -1 && index==range.length-1){
...
TREE.prototype.addToArray = function (obj,args){
...
TREE.prototype.metaBalls = {
	...
	updateBalls:function(arr) {
		...
		if(arr==undefined){
			...
			ballArr = this.worldPositionsArray(report);
