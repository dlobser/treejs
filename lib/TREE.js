...
// TREE.prototype.makeList2 = function(range) {
...
// 		var min, max;
// 		var newResult = [];
// 		console.log(range[i] + " " + i%2 + " " +i+" "+ result.length + " " + range.length);
// 		if (range[i]=="all"&&i%2==0&&result.length>0&&i!=range.length-1&&i!=0){
// 			var j = [];
// 			var tempResult = [];
// 			for (var i = 0; i < result.length; i++) {
// 				tempResult[i] = result[i][result.length-1];
// 			};
// 			tempResult.push(0);
// 			// console.log(tempResult);
// 						console.log(tempResult);

// 			this.findLimbs(this.FIND(tempResult),j);
...
TREE.prototype.reportMirrorLayers = function(obj,stack,stackArray,pusher){
	...
	for(var i = 0 ; i < joint.limbs.length ; i++){
		...
		for(var k = 0 ; k < t4.length ; k++){
			...
			this.parts[tempString] = this.FIND(t4[k]);
		...
		for(var j = 0 ; j < jarr.length ; j++){
			this.reportMirrorLayers(jarr[j],tempStack,stackArray,j);
		}
	}
...
TREE.prototype.passFunc2 = function (array,func,GPU){
	...
	for (var i = 0; i < array.length; i++) {
		...
		//  	for (var k = 0; k < process.length; k++) {
		 		if(accelerated){
		 			...
		 			// );
					func(this.bones[this.Move(array[i][j],this.utils.findInMirrorArray,this.mirrorJointArray)],array[i][j].args);

		 		}
		 		else{
		 			// var start = performance.now();
					this.Move(array[i][j],func,array[i][j].args);
					// var end = performance.now();
					// console.log(end-start);
			// 	}
			};
