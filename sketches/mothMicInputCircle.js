...
sc1 = {
    ...
    setup:function(){
        ...
        var geo = new THREE.PlaneGeometry(200,200,10,10);
        ball2 = new THREE.Mesh(geo, new THREE.MeshLambertMaterial({color:0xFFFFFF,transparent:true,opacity:1, map:gradMap,emissive:0xffffff}));
        ball.position.z=30;
        scene.add(ball2);
        ...
        scene.add(new THREE.PointLight(new THREE.Color(0x99eeff),4,200));
        scene.add(new THREE.PointLight(new THREE.Color(0x99eeff),4,200));
        scene.add(new THREE.PointLight(new THREE.Color(0x99eeff),4,200));
        ...
        for(var i = 0 ; i < 32 ; i++){
            ...
            moths[i] = cloner(mth);
            moths[i].rotation.z=pi*2;
            p.add(moths[i])
            parents[i].add(p);

            var cyl = new THREE.CylinderGeometry( .2,.2, 200,6,1);
            var Cyl = new THREE.Mesh(cyl, new THREE.MeshLambertMaterial({color:0x222222}));
            Cyl.rotation.x = pi;
            // Cyl.position.z = 50;
            p.add(Cyl);
            ...
            moths[i].position.z = 100;
            parents[i].rotation.y = (i/32)*(pi*4);
            p.position.z = 120;
    ...
    draw:function(time){
        ...
        for(i = 0 ; i < audioValues.length ; i++){
          a+=audioValues[i];
        }
        a/=audioValues.length;
        a-=128;
        a*=.02;
        ball.scale = new THREE.Vector3(a,a,a);

        var wh = audioValues[0]-127.5;
        if(wh>1)
          avgB = avg.avg(wh);
        else
          avgB = avg.avg(0);
        ...
        for(var i = 0 ; i < moths.length ; i++){
            ...
            var trans = new THREE.Vector3(noise(cnt+(i*.1))*amt*10,noise(cnt+(i*.1)+.3)*amt*10,noise(cnt+(i*.1)+.6)*amt*10);
            ...
            // mth.rotation.x+=trans.x*.001;
            // mth.rotation.y+=trans.y*.001;
            // mth.rotation.z+=trans.z*.001;

            if(audioValues[0]){
                var amt = (Math.abs(audioValues[i*32])-128)*.1;
                parents[i].children[0].rotation.x = amt*amt*.5;
                // var sc = (audioValues[i*32]-128)*.01;
                // ball.scale = new THREE.Vector3(sc,sc,sc);
            }

            var amt = 10+wh;

            // mth.position.x=(.5-Math.random())*amt*10;
            // mth.position.y=(.5-Math.random())*amt*10;
            // mth.position.z=(.5-Math.random())*amt*10;

            // mth.position = trans;



            mth.scale = new THREE.Vector3(.5,.5,.5);
            // mth.position.y = Math.cos(1+i+time*speed)*13;
            mth.p.ULWing.bones[1].rotation.y =     Math.cos(i+cnt*speed)*.5;
            mth.p.ULWing.bones[0].rotation.y =     Math.sin(i+cnt*speed);
            mth.p.URWing.bones[1].rotation.y =    -Math.cos(i+cnt*speed)*.5;
            mth.p.URWing.bones[0].rotation.y =    -Math.sin(i+cnt*speed);
            mth.p.LLWing.bones[1].rotation.y =( Math.cos(.2+i+cnt*speed)*.95)*.5;
            mth.p.LLWing.bones[0].rotation.y =( Math.sin(.2+i+cnt*speed)*.95)-.2;
            mth.p.LRWing.bones[1].rotation.y =(-Math.cos(.2+i+cnt*speed)*.95)*.5;
            mth.p.LRWing.bones[0].rotation.y =(-Math.sin(.2+i+cnt*speed)*.95)+.2;

            mth.p.leftAnt.bones[0].rotation.z =     .2;
            mth.p.rightAnt.bones[0].rotation.z =    -.2;

            mth.p.leftAnt.bones[1].rotation.x =     Math.sin(i+cnt*speed)*.2;
            mth.p.rightAnt.bones[1].rotation.x =    Math.sin(i+cnt*speed)*.2;

            for(var k = 0 ; k < mth.p.legs.length ; k++){
                 mth.p.legs[k].bones[1].rotation.z =     Math.sin(i+cnt*speed)*.2;
            }
            mth.p.body.bones[1].rotation.z = Math.sin(1+cnt*speed*2)*.1;
            mth.p.body.bones[1].rotation.x = Math.sin(cnt*speed)*.5;
        }
