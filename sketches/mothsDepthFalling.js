...
sc1 = {
    ...
    setup:function(){
        ...
        lgts.add(new THREE.PointLight(new THREE.Color(0x99eeff),2,100));
        lgts.add(new THREE.PointLight(new THREE.Color(0x99eeff),2,100));
        lgts.add(new THREE.PointLight(new THREE.Color(0x99eeff),2,100));
        ...
        rgbEffect.uniforms[ 'far' ].value = 0.5;//0015;
        ...
        for(var i = 0 ; i < 40 ; i++){
            ...
            scene.add(parents[i]);
            moths[i].position.x = (Math.random()-.5)*250;
            moths[i].position.z = (Math.random()-.5)*100;

            moths[i].position.y = (Math.random()-.5)*500;
    ...
    draw:function(time){
        ...
        for(var i = 0 ; i < moths.length ; i++){
            ...
            mth.rotation.x+=Math.random()*.02;
            mth.rotation.y+=Math.random()*.02;
            mth.rotation.z+=Math.random()*.02;

            // mth.position.x=(.5-Math.random())*300;
            mth.position.y-=.05*10;
            if(mth.position.y<-200)
                mth.position.y=200;
            ...
            mth.p.leftAnt.bones[1].rotation.x =     Math.sin(i+time*speed)*.2;
            mth.p.rightAnt.bones[1].rotation.x =    Math.sin(i+time*speed)*.2;

            for(var k = 0 ; k < mth.p.legs.length ; k++){
                 mth.p.legs[k].bones[1].rotation.z =     Math.sin(i+time*speed)*.2;
            }
